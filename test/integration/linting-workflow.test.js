...
describe('Linting Workflow Integration', () => {
  ...
  describe('Custom Rules Integration', () => {
    it('should load and execute custom rules', async () => {
      ...
      const hasDescriptionRule = results.some((result) => result.message && result.message.includes('description'));
      const hasNamingRule = results.some((result) => result.message && result.message.includes('__c'));
